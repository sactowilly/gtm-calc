...
Backups contain sensitive customer/pricing data and carry a permanent warning. Validation checks the envelope, checksum, supported schema, unique IDs/numbers, references, immutable hashes, field types/ranges, and record counts before any write. Version 2.5 PR 2 downloads the readable, revalidated envelope as an unencrypted UTF-8 JSON file named from `exportedAt`. PR 3 keeps a selected file only in page memory while it produces a sanitized, aggregate inspection report; it adds no entity, persisted record, or restore mutation. Restore policies remain later gated operations.
