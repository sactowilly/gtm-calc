...
- Version 2 is complete and tagged `v2.0.0` at verified production commit `b61890c`. Version 2.5 is complete and tagged `v2.5.0` at verified production commit `7ab4d2e`. Version 3.0 is in progress with development marker `v3.0.0 · offline-shell.3` and package version `3.0.0-alpha.3`.
...
- Physical Version 2.5 backup-download/restore evidence, owner acceptance, post-merge Pages verification, and annotated tag publication are complete. Version 3.0 now adds `manifest.webmanifest`, branded square/maskable icons, installation guidance, a `/gtm-calc/`-scoped worker, a versioned cache of public application-shell files, and an explicit online/offline status. After one successful online launch, the calculator and existing browser-local quote/library/catalog data reopen offline. The worker never caches or changes PDFs, backups, mailto links, IndexedDB, localStorage, customer records, or quote pricing. Safe update activation and physical device acceptance remain later work; deletion/archive, backend, authentication, synchronization, and hosted access remain deferred.
