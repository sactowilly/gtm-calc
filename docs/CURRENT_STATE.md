...
- Version 2 is complete and tagged `v2.0.0` at verified production commit `b61890c`. Version 2.5 is now in progress with marker `v2.5.0 · restore-transaction.4` and package version `2.5.0-alpha.4` on its current feature branch.
- The Version 2.5 foundation adds a readonly, single-transaction snapshot of all eight `gtm_quote_manager` stores plus scoped capture of the active quote, active/prior catalogs, manual items, catalog usage, and recovery localStorage records. It adds deterministic SHA-256 envelope validation, reference checks, and immutable-version hash verification. PR #24 merged the Export workspace's exact-byte-validated local JSON download, and PR #25 merged the pre-read 25 MiB inspection, strict UTF-8/JSON/envelope validation, readonly current-device comparison, and sanitized no-write conflict preview. The current slice revalidates a selected file before an explicit owner-confirmed Merge or Replace, requests a safety-backup download before local writes, uses a transaction/rollback path, and validates the resulting local state without exposing raw customer, pricing, or record-ID data in the UI.
- CSV/individual exports, deletion/archive, PWA, backend, authentication, synchronization, and hosted access remain deferred according to the roadmap and `V25_IMPLEMENTATION_PLAN.md`.
