...
- Version 2 is complete and tagged `v2.0.0` at verified production commit `b61890c`. Version 2.5 is now in progress with marker `v2.5.0 · record-exports.5` and package version `2.5.0-alpha.5` on its current feature branch.
...
- Physical Version 2.5 backup-download evidence, release hardening, deletion/archive, PWA, backend, authentication, synchronization, and hosted access remain deferred according to the roadmap and `V25_IMPLEMENTATION_PLAN.md`.
...
This is browser/profile/origin-local data. It does not synchronize across devices, private browsing may discard it, and clearing site data deletes it. The Version 2.5 feature branch now adds an Export workspace that validates and downloads a complete, unencrypted JSON backup of persisted application data. Restore remains unavailable until a later planned pull request.
