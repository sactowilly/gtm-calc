...
## Current implementation update — 2026-08-06
...
- Vite/native ES modules, Vitest, Playwright, and pull-request GitHub Actions are present.
- Version 1 phone-first item cards, customer/quote details, Sales Rep/UOM/lead-time fields, local customer-safe PDF generation, download/share fallbacks, and separate rep/customer email paths are implemented.
...
- Version 2 is complete and tagged `v2.0.0` at verified production commit `b61890c`. Version 2.5 is complete and tagged `v2.5.0` at verified production commit `7ab4d2e`. Version 3.0 is tagged `v3.0.0` at verified production commit `a908398`. Version 3.5 is in progress with marker/package `v3.5.0 · inline-search.1` / `3.5.0-alpha.1`.
- The Version 2.5 foundation adds a readonly, single-transaction snapshot of all eight `gtm_quote_manager` stores plus scoped capture of the active quote, active/prior catalogs, manual items, catalog usage, and recovery localStorage records. It adds deterministic SHA-256 envelope validation, reference checks, and immutable-version hash verification. PR #24 merged the Export workspace's exact-byte-validated local JSON download, PR #25 merged the pre-read 25 MiB inspection and sanitized no-write conflict preview, and PR #26 merged the revalidated owner-confirmed Merge/Replace transaction with a pre-write safety backup, rollback path, and post-restore validation. The current PR adds RFC 4180/formula-safe quote, customer, and manual-item CSV reports plus individual quote JSON and existing customer-safe PDF downloads from saved quote cards. No raw customer, pricing, or record-ID data is exposed in the export status UI.
- Physical Version 2.5 backup-download/restore evidence, owner acceptance, post-merge Pages verification, and annotated tag publication are complete. Version 3.0 adds `manifest.webmanifest`, branded square/maskable icons, installation guidance, a `/gtm-calc/`-scoped worker, a versioned cache of public application-shell files, explicit online/offline status, a non-blocking safe-update notice, and a direct customer-PDF Blob link for browsers whose embedded viewer cannot open a PDF. After one successful online launch, the calculator and existing browser-local quote/library/catalog data reopen offline. A user must explicitly choose Reload update; unsaved quote edits require confirmation and active restore work blocks update activation. The worker never caches or changes PDFs, backups, mailto links, IndexedDB, localStorage, customer records, or quote pricing. Owner Android Chrome and laptop Chromium acceptance is recorded; iPhone Safari physical testing is explicitly deferred by the owner. Version 3.5 now adds inline local Catalog/My Items search in the Item field without leaving the Quote workspace; backend, authentication, synchronization, and hosted access remain deferred.
...
This is browser/profile/origin-local data. It does not synchronize across devices, private browsing may discard it, and clearing site data deletes it. Version 2.5 adds an Export workspace that validates and downloads a complete, unencrypted JSON backup of persisted application data, plus inspected and owner-confirmed Merge/Replace restore. Stable release tagging remains gated by the closeout checklist.
