...
- Version 2 is complete and tagged `v2.0.0` at verified production commit `b61890c`. Version 2.5 is complete and tagged `v2.5.0` at verified production commit `7ab4d2e`. Version 3.0 is in progress with development marker `v3.0.0 · shell-cache.2` and package version `3.0.0-alpha.2`.
...
- Physical Version 2.5 backup-download/restore evidence, owner acceptance, post-merge Pages verification, and annotated tag publication are complete. Version 3.0 now adds `manifest.webmanifest`, branded square/maskable icons, installation guidance, a `/gtm-calc/`-scoped worker, and a versioned cache containing only public bootstrap files. It has no fetch interception, offline workflow claim, cache of PDFs/backups/customer data, or stored-data change. Later PWA work follows `V3_IMPLEMENTATION_PLAN.md`; deletion/archive, backend, authentication, synchronization, and hosted access remain deferred.
