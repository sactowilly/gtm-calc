...
- Version 2 is complete and tagged `v2.0.0` at verified production commit `b61890c`. Version 2.5 is complete and tagged `v2.5.0` at verified production commit `7ab4d2e`. Version 3.0 is in progress with development marker `v3.0.0 · install-metadata.1` and package version `3.0.0-alpha.1`.
...
- Physical Version 2.5 backup-download/restore evidence, owner acceptance, post-merge Pages verification, and annotated tag publication are complete. The first Version 3.0 runtime slice adds `manifest.webmanifest`, branded square/maskable icons, theme/home-screen metadata, installation guidance, and direct-source/production manifest checks. It adds no service worker, fetch interception, cache, offline claim, or stored-data change. Later PWA work follows `V3_IMPLEMENTATION_PLAN.md`; deletion/archive, backend, authentication, synchronization, and hosted access remain deferred.
