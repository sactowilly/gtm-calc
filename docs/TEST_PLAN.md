...
PR #19 release-hardening coverage additionally requires:

- Generate a customer PDF, edit quote/customer data, and prove the next preview/download uses regenerated bytes.
- Prove Copy Internal contains the approved rep detail while Copy Customer Quote excludes cost, freight cost, GTM/markup, source IDs, and internal notes.
- Cancel and accept same-draft Reopen with dirty data, including a pending item-form entry.
- Inject an initial draft-creation failure and prove customer, contact, quote, and event writes all roll back.
- Exercise 100 newest-first records with ten-at-a-time disclosure, an older long Unicode record, search, wrapping, and 320 px overflow checks.
- Serve the untransformed repository source and the built `dist/` artifact under `/gtm-calc/`; assert correct marker, local persistence, no missing assets, and no browser/page errors.
- Keep the package marker, README, current state, product roadmap, V2 plan, open items, and roadmap SVG wording consistent through `tests/release-metadata.test.js`.
- Record physical Android Chrome, laptop Edge, rollback/re-entry, and post-merge production acceptance in `docs/V2_RELEASE_CHECKLIST.md`.
...
Implementation status (2026-08-06): the Version 2.5 foundation covers consistent all-store snapshots, scoped localStorage capture, stable ordering/checksums, prototype-safe canonicalization, unsupported schema/duplicate/reference rejection, payload tampering, and immutable-version hash verification. The merged download slice adds exact serialized-byte revalidation, UTF-8 Blob/MIME and byte-size checks, deterministic UTC filenames, temporary object-URL cleanup, truthful success/failure states, real browser download parsing, five-workspace navigation, and before/after storage equality. PR #25 adds a pre-read 25 MiB guard, strict UTF-8/JSON/envelope validation, sanitized conflict analysis, and no-write preview coverage. PR #26 revalidates before commit, requests a safety-backup download, exercises merge/replace policies, verifies rollback/post-restore state, and locks/reloads stale UI memory after success. PR #27 adds RFC 4180/formula-safe CSV reports, individual JSON/PDF projections, immutable-version selection, deterministic local downloads, and five-profile browser coverage. PR #28 records owner physical acceptance on Android Chrome and laptop Chromium plus post-merge Pages verification; annotated tag `v2.5.0` identifies verified production commit `7ab4d2e`.
...
- Restore-inspection tests reject a declared file over 25 MiB before read/readiness/current snapshot, decode UTF-8 strictly, reject malformed/tampered/unsupported input, and validate a safe incoming file before snapshotting current data.
- Preview tests use real browser file selection, show only aggregate metadata/counts/conflicts, treat altered immutable versions/events and finalized-number collisions as blocking, expose no customer/pricing/raw-file data, and prove every inspection/cancel/retry path leaves IndexedDB, localStorage, sessionStorage, active quote, and network state unchanged.
...
- Merge identical skip, mutable conflict, and immutable/number collision reports; revalidate the selected file/current snapshot again at commit time.
- Replace creates a downloaded safety backup before any write, commits atomically, and rollback leaves old data on injected failure.
- Successful restore locks the pre-restore application UI and reloads persisted state before an old in-memory quote can be saved.
- CSV exports quote/customer/manual item columns, Unicode/quotes/newlines, and protect spreadsheet formula injection.
- CSV report UI requests quote-list, customer, and manual-item downloads with truthful status/error recovery and leaves IndexedDB/localStorage/sessionStorage unchanged.
- Individual draft JSON exports include only the selected draft; finalized JSON/PDF exports require the selected immutable version, preserve historical display numbers, use deterministic filenames, and keep customer PDF fields behind the existing privacy allowlist.
...
- Owner acceptance, closeout deployment verification, and annotated tag publication passed on 2026-08-06. Tag `v2.5.0` points to verified production commit `7ab4d2e`.

### Version 2.5 closeout evidence

- PR #28 merged as `0716350` after the closeout documentation and release checklist were reviewed.
- GitHub Pages deployment run `31128417026` passed for the merged commit; the live `/gtm-calc/` site returned HTTP 200 and served `v2.5.0 · release-closeout.6`.
...
Implementation status (2026-09-08): Version 3 is complete. The manifest/install-metadata slice verifies manifest JSON, `/gtm-calc/` ID/start/scope, standalone display, theme/background colors, 180/192/512 PNG dimensions, a 512 maskable icon, and source/build asset loading. The cache-policy slice verifies scoped registration, non-fatal unsupported/failed registration, `updateViaCache: 'none'`, and retirement of only application-owned caches. The offline-shell slice verifies that only public HTML/CSS/JS/vendor/logo/icon assets are precached, sensitive output is bypassed, both direct-source Pages and built artifacts reload offline after one online launch, existing local quote data reopens, calculator entry remains usable, and the visible connection status changes offline. The safe-update slice verifies a waiting worker never activates itself; reload requires explicit user action, unsaved quote edits require confirmation, active restore work blocks activation, controller changes reload once, deferred notices are non-destructive, and only stale application-owned caches retire. The PDF-open fallback verifies the direct customer-PDF Blob link is visible only after generation and uses a new-tab link without relying on embedded-viewer controls. The hidden-control regression reuses the 100-draft pagination test across Chromium, Firefox, WebKit, Android Chrome, and iPhone Safari to prove an exhausted `Show 10 more` button is not visible. PR #39 GitHub Actions and the resulting Pages deployment passed; owner Android Chrome/laptop Chromium acceptance is recorded and iPhone Safari is explicitly deferred. PDFs, backups, mailto URLs, IndexedDB, and localStorage remain outside the worker cache.

## Version 3.5 verification

Implementation status (2026-09-08): the inline Item-field search suite imports a local catalog, searches from the Quote workspace, verifies keyboard ArrowDown/Enter selection and touch selection without navigation to Catalog, preserves catalog source/SKU when a line item is saved, verifies confirmation protects edited UOM/cost/price/lead-time values, checks the no-result manual-entry fallback, and asserts no phone-width horizontal overflow. The new suite runs across Chromium, Firefox, WebKit, Android Chrome emulation, and iPhone Safari emulation. Existing Catalog workspace import/search/My Item/dirty-guard coverage continues to run unchanged.
