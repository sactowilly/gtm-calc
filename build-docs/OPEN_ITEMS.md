...
| Version 3.0 PWA implementation | in progress | The `safe-update.4` slice adds an explicit waiting-update notice and controlled activation/reload, with unsaved-edit confirmation and active-restore blocking. The worker never caches customer/business data, PDFs, backups, mailto links, IndexedDB, or localStorage. Next: physical device acceptance and production closeout. |
