...
| Version 2.5 backup and restore | in progress | PR 1 backup foundation is merged. PR 2 adds the local Export workspace and validated complete JSON download. Restore inspection/writes, CSV, and individual exports remain later gated PRs. |
| Version 2.5 PR 2 physical acceptance | pending before merge | On Android Chrome and laptop Chromium, verify the five-tab navigation, warning, actual downloaded JSON/filename, offline behavior, repeated activation, keyboard behavior, and unchanged saved data. |
