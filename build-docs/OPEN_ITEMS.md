...
| Version 3.0 PWA implementation | in progress | PR #38's `pdf-open.5` slice adds an application-owned direct PDF Blob link because Android Chrome's embedded viewer Open control was unreliable. Its full matrix exposed a visual-hidden regression in quote-library pagination; `hidden-controls.6` corrects that before physical retesting. Next: successful CI, Android retest, iPhone Safari acceptance or an explicit approved deferral, laptop confirmation, and production closeout. |
