...
| Version 3.0 PWA implementation | in progress | The `install-metadata.1` slice adds the `/gtm-calc/` manifest, branded icons, install guidance, and source/build tests without a service worker. Next: explicit service-worker registration and cache-policy PR. |
