...
| Version 3.0 PWA implementation | complete | PR #40 merged as `a908398`; Pages deployment `34286988078` passed and immutable annotated tag `v3.0.0` identifies that production commit. Owner Android Chrome/laptop Chromium acceptance is recorded; iPhone Safari is explicitly deferred. |
| Version 3.5 mobile workflow | in progress | First slice: inline catalog/My Items search from the Quote workspace Item field, reducing catalog-page context switching while preserving dirty line-item input, privacy boundaries, and the standalone Catalog workflow. |
