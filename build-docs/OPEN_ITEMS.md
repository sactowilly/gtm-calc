...
| Version 3.0 PWA implementation | in progress | The `pdf-open.5` slice adds an application-owned direct PDF Blob link because Android Chrome's embedded viewer Open control was unreliable. It retains controlled update activation, local-only storage, and customer-output privacy. Next: Android retest, iPhone Safari acceptance or an explicit approved deferral, laptop confirmation, and production closeout. |
