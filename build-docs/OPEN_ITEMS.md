...
| Version 3.0 PWA implementation | in progress | The `offline-shell.3` slice adds offline reopen after one successful online launch for the public application shell and existing browser-local calculator/library data. The worker never caches customer/business data, PDFs, backups, mailto links, IndexedDB, or localStorage. Next: safe update activation, then physical device acceptance. |
