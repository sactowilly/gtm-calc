...
const buildLabel = `v3.0.0 ${String.fromCharCode(0x00b7)} hidden-controls.6`;

describe('Version 3 development metadata', () => {
  it('keeps the Version 3 hidden-controls marker and package version aligned', () => {
    const packageMetadata = JSON.parse(read('package.json'));
    expect(APP_BUILD_LABEL).toBe(buildLabel);
    expect(packageMetadata.version).toBe('3.0.0-alpha.6');
